...
# Stop local webmin from blocking port 10000
	...
	# minecraft server
	chroot $1 mkdir /minecraft
	chroot $1 wget -q https://s3.amazonaws.com/Minecraft.Download/versions/1.9/minecraft_server.1.9.jar -O /minecraft/minecraft_server.jar

	# MCMyAdmin
	#chroot $1 wget http://mcmyadmin.com/Downloads/etc.zip -O /usr/local
	#chroot $1 cd /usr/local; unzip etc.zip
	#chroot $1 mkdir /mcmyadmin
	#chroot $1 wget http://mcmyadmin.com/Downloads/MCMA2_glibc26_2.zip -O /mcmyadmin
	#chroot $1 cd /mcmyadmin; unzip MCMA2_glibc26_2.zip
...
# First exclude all, then include all the modules we want installed for this app
	...
	tar rvf $dname.wbm.tar origo/tabs/security origo/tabs/software origo/tabs/servers origo/tabs/minecraft
...
# Utility script for setting up WordPress to work with this app
    cp origo-minecraft.sh $1/usr/local/bin
    chmod 755 $1/usr/local/bin/origo-minecraft.sh
    chroot $1 bash -c 'echo "start on (started origo-networking)
task
exec /usr/local/bin/origo-minecraft.sh" > /etc/init/origo-minecraft.conf'
