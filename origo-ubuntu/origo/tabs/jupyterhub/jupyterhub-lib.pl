  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-body">
        ...
        <form id="edit_user_form" class="small" method="post" action="index.cgi?action=savesambauser\&tab=jupyterhub" autocomplete="off">
...
//            scrollY: 280,
            ...
            ajax: {
                url: "index.cgi?tab=jupyterhub\&action=listsambausers",
                dataSrc: ""
            }
        });
    });
...
//                                        \$('#urglyph').attr('class','glyphicon glyphicon-repeat');
    ...
    function saveSambaUser(user) {
        ...
        \$.post( "index.cgi?action=savesambauser\&tab=jupyterhub", \$("#edit_user_form").serialize())
        .done(function( data ) {
            salert(data);
            updateSambaUsers();
        })
    ...
    function deleteSambaUser() {
        ...
        \$.post( "index.cgi?action=deletesambauser\&tab=jupyterhub", \$("#edit_user_form").serialize())
        .done(function( data ) {
            salert(data);
            updateSambaUsers();
        })
