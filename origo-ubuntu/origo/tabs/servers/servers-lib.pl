...
<button class="btn btn-default" id="add_servers" title="Click to add servers to your app. Type in the number of servers you want to add in the text field." rel="tooltip" data-placement="bottom" onclick="\$('[rel=tooltip]').tooltip('hide'); addServers('', \$('#n').val()); return false;"><span class="glyphicon glyphicon-plus"></span></button>
<button class="btn btn-default" id="remove_servers" title="Click to remove servers from your app. Type in the number of servers you want to remove in the text field." rel="tooltip" data-placement="bottom" onclick="\$('[rel=tooltip]').tooltip('hide'); addServers(1, \$('#n').val()); return false;"><span class="glyphicon glyphicon-minus"></span></span></button>
...
sub create_simplestack_server {
    ...
    my $cmd = qq[curl -ks "https://10.0.0.1/steamengine/systems?action=buildsystem&monitors=ping&start=1$nstr"];
    my $json_text = `$cmd`;
...
sub delete_simplestack_server {
    ...
    my $cmd = qq[curl -ks -X POST --data-urlencode 'POSTDATA=$postdata' https://10.0.0.1/steamengine/servers];

    my $reply = `$cmd`;

    my $putdata = qq/{"uuid": "$uuid", "action": "deletesystem"}/;
    $cmd = qq[curl -ks -X PUT --data-urlencode 'PUTDATA=$putdata' https://10.0.0.1/steamengine/systems];
    $reply .= `$cmd`;
