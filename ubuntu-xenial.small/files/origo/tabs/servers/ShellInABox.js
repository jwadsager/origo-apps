...
//    console.log(keys);
    if (keys == '1B37') keys = '5C'; // map shift-7 to \ in Chrome DK
    else if (keys == '1B49') keys = '7C'; // transate alt-I to | in Firefox DK
//    else if (keys.indexOf('1B')==0 && keys.length>2) keys=keys.substring(2); // | and @ in Chrome DK
    else if (keys=='1B7C' || keys=='1B40') keys=keys.substring(2); // | and @ in Chrome DK
    request.open('POST', this.url + '?', true);
    request.setRequestHeader('Cache-Control', 'no-cache');
    request.setRequestHeader('Content-Type',
                           'application/x-www-form-urlencoded; charset=utf-8');
